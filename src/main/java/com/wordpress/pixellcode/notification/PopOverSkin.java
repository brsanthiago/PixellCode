...
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.BOTTOM_CENTER;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.BOTTOM_LEFT;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.BOTTOM_RIGHT;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.LEFT_BOTTOM;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.LEFT_CENTER;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.LEFT_TOP;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.RIGHT_BOTTOM;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.RIGHT_CENTER;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.RIGHT_TOP;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.TOP_CENTER;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.TOP_LEFT;
import static com.wordpress.pixellcode.notification.PopOver.ArrowLocation.TOP_RIGHT;
...
public class PopOverSkin implements Skin<PopOver> {
	...
	public PopOverSkin(final PopOver popOver) {
		...
		stackPane.getStylesheets().add(PopOver.class.getResource("/css/popover.css").toExternalForm()); //$NON-NLS-1$
		stackPane.setPickOnBounds(false);
		stackPane.getStyleClass().add("snack"); //$NON-NLS-1$

		/*
		 * The min width and height equal 2 * corner radius + 2 * arrow indent +
		 * 2 * arrow size.
		 */
		stackPane.minWidthProperty()
				.bind(Bindings.add(Bindings.multiply(2, popOver.arrowSizeProperty()),
						Bindings.add(Bindings.multiply(2, popOver.cornerRadiusProperty()),
	...
	private void createPathElements() {
		...
		bottomEdgeMinusRadiusProperty
				.bind(Bindings.subtract(bottomEdgeProperty, getSkinnable().cornerRadiusProperty()));
		...
		lineHTop.xProperty()
				.bind(Bindings.subtract(Bindings.subtract(rightEdgeMinusRadiusProperty, arrowIndentProperty),
						Bindings.multiply(arrowSizeProperty, 2)));

		lineITop = new LineTo();
		lineITop.xProperty().bind(Bindings
				.subtract(Bindings.subtract(rightEdgeMinusRadiusProperty, arrowIndentProperty), arrowSizeProperty));
		lineITop.yProperty().bind(Bindings.subtract(topEdgeProperty, arrowSizeProperty));

		lineJTop = new LineTo();
		lineJTop.xProperty().bind(Bindings.subtract(rightEdgeMinusRadiusProperty, arrowIndentProperty));
		...
		lineHRight.yProperty()
				.bind(Bindings.subtract(Bindings.subtract(bottomEdgeMinusRadiusProperty, arrowIndentProperty),
						Bindings.multiply(arrowSizeProperty, 2)));

		lineIRight = new LineTo();
		lineIRight.xProperty().bind(Bindings.add(rightEdgeProperty, arrowSizeProperty));
		lineIRight.yProperty().bind(Bindings
				.subtract(Bindings.subtract(bottomEdgeMinusRadiusProperty, arrowIndentProperty), arrowSizeProperty));

		lineJRight = new LineTo();
		lineJRight.xProperty().bind(rightEdgeProperty);
		lineJRight.yProperty().bind(Bindings.subtract(bottomEdgeMinusRadiusProperty, arrowIndentProperty));
		...
		lineHBottom.xProperty().bind(Bindings.add(Bindings.add(leftEdgePlusRadiusProperty, arrowIndentProperty),
				Bindings.multiply(arrowSizeProperty, 2)));

		lineIBottom = new LineTo();
		lineIBottom.xProperty()
				.bind(Bindings.add(Bindings.add(leftEdgePlusRadiusProperty, arrowIndentProperty), arrowSizeProperty));
		...
		lineHLeft.yProperty().bind(Bindings.add(Bindings.add(topEdgePlusRadiusProperty, arrowIndentProperty),
				Bindings.multiply(arrowSizeProperty, 2)));

		lineILeft = new LineTo();
		lineILeft.xProperty().bind(Bindings.subtract(leftEdgeProperty, arrowSizeProperty));
		lineILeft.yProperty()
				.bind(Bindings.add(Bindings.add(topEdgePlusRadiusProperty, arrowIndentProperty), arrowSizeProperty));
